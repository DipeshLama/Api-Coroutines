...
class HomeRepository @Inject constructor(private val appDatabase : AppDatabase) : BaseRepository() {
    ...
    suspend fun getHomeScreenData() = apiService.getHomeScreenData(1)

    suspend fun saveHomeScreenDataToDb (list: List<Home>) = appDatabase.getHomeDao().insert(list)

    fun getHomeScreenDataFromRoom () = appDatabase.getHomeDao().retrieve()
}